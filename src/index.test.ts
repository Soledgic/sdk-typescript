...
describe('Soledgic SDK', () => {
  ...
  it('defaults to the public API base URL when baseUrl is missing', async () => {
    ...
    const sdk = new Soledgic({ apiKey: API_KEY })
  ...
  it('normalizes root API base URLs to the versioned API root', async () => {
    expect(normalizeBaseUrl('https://api.soledgic.com')).toBe('https://api.soledgic.com/v1')
    expect(normalizeBaseUrl('https://api.soledgic.com/')).toBe('https://api.soledgic.com/v1')
  })

  it('keeps versioned API roots stable and avoids duplicate /v1 suffixes', async () => {
    expect(normalizeBaseUrl('https://api.soledgic.com/v1')).toBe('https://api.soledgic.com/v1')
    expect(normalizeBaseUrl('https://api.soledgic.com/v1/')).toBe('https://api.soledgic.com/v1')
    expect(normalizeBaseUrl('https://api.soledgic.com/v1/v1/')).toBe('https://api.soledgic.com/v1')
  })

  it('uses normalized root API base URLs for requests', async () => {
    const fn = mockFetch({ success: true, periods: [] })
    vi.stubGlobal('fetch', fn)
    const sdk = new Soledgic({ apiKey: API_KEY, baseUrl: 'https://api.soledgic.com' })
    await sdk.listPeriods()
    const [url] = fn.mock.calls[0]
    expect(url).toBe('https://api.soledgic.com/v1/close-period')
  })
  ...
  it('users.upsertWallet maps external users to consumer wallets', async () => {
    const fn = mockFetch({
      success: true,
      created: true,
      wallet: {
    ...
    await sdk.users.upsertWallet({
      externalUserId: 'user_1',
      name: 'User wallet',
      metadata: { source: 'booklyverse' },
    })

    const [url, opts] = fn.mock.calls[0]
    const body = JSON.parse(opts.body)
    expect(url).toBe(`${BASE_URL}/wallets`)
  ...
  it('creators.upsert maps external creators to participants', async () => {
    ...
    await sdk.creators.upsert({
      externalCreatorId: 'creator_1',
    ...
    expect(body).toMatchObject({
      ...
      default_split_percent: 80,
      metadata: {
        external_creator_id: 'creator_1',
      },
    })
  })

  it('orders.createCheckout maps order/product/user vocabulary to checkout sessions', async () => {
    const fn = mockFetch({
      success: true,
      checkout_session: {
        id: 'cs_1',
        mode: 'session',
        checkout_url: 'https://checkout.example/session/cs_1',
        amount: 2500,
        currency: 'USD',
      },
    })
    ...
    await sdk.orders.createCheckout({
      ...
      successUrl: 'https://booklyverse.example/success',
      metadata: { app: 'booklyverse' },
    })
    ...
    expect(body).toMatchObject({
      ...
      success_url: 'https://booklyverse.example/success',
      metadata: {
        app: 'booklyverse',
  ...
  it('walletSessions.create maps hosted wallet sessions', async () => {
    ...
    const result = await sdk.walletSessions.create({
      ...
      permissions: ['view_balance', 'list_activity'],
      successUrl: 'https://merchant.example/done',
      cancelUrl: 'https://merchant.example/account',
      expiresInMinutes: 30,
      idempotencyKey: 'wallet_session_user_1',
      metadata: { app: 'merchant' },
    })

    const [url, opts] = fn.mock.calls[0]
    const body = JSON.parse(opts.body)
    expect(String(url)).toBe(`${BASE_URL}/wallet-sessions`)
    expect(body).toMatchObject({
      external_user_id: 'user_1',
      customer_email: 'buyer@example.com',
      permissions: ['view_balance', 'list_activity'],
  ...
  it('walletSessions.create sends participant owner type for creator earnings sessions', async () => {
    const fn = mockFetch({
      ...
      wallet_session: {
        ...
        created_at: '2026-05-12T11:30:00Z',
        metadata: {
          session_type: 'creator_earnings',
          wallet_account_type: 'creator_balance',
    ...
    const result = await sdk.walletSessions.create({
      ...
      permissions: ['view_balance', 'list_activity'],
      successUrl: 'https://merchant.example/creator',
      idempotencyKey: 'wallet_session_creator_123',
    })
    ...
    expect(body).toMatchObject({
      ...
      permissions: ['view_balance', 'list_activity'],
      success_url: 'https://merchant.example/creator',
      idempotency_key: 'wallet_session_creator_123',
    })
    expect(result.walletSession.metadata).toMatchObject({
      session_type: 'creator_earnings',
      wallet_account_type: 'creator_balance',
      participant_id: 'creator_123',
    })
  ...
  it('payouts, refunds, receipts, holds, reversals, and activity expose resource-first aliases', async () => {
    ...
    const receiptFn = mockFetch({ success: true, receipt: { id: 'rcpt_1', receipt_id: 'rcpt_1', status: 'created', hosted_url: 'https://soledgic.com/receipts/rcpt_1', metadata: {} } })
    vi.stubGlobal('fetch', receiptFn)
    await sdk.receipts.generate({ checkoutId: 'chk_1', externalOrderId: 'order_1' })
    expect(String(receiptFn.mock.calls[0][0])).toContain('/receipts/generate')

    const activityFn = mockFetch({ success: true, wallet: null, entries: [], total: 0, limit: 10, offset: 0 })
  ...
  describe('typed error classes', () => {
    ...
    it('throwTypedError maps status codes to correct error classes', async () => {
      for (const [status, ErrorClass] of [
        ...
        const sdk = createClient(fn)
        try {
          await sdk.createPayout({ participantId: 'p', amount: 100, referenceId: 'r' })
  ...
  it('sandbox helpers map checkout simulation and webhook test routes', async () => {
    const completeFn = mockFetch({
      success: true,
      checkout_session: {
        id: 'sess_1',
        status: 'completed',
        mode: 'session',
        payment_id: 'sandbox_pay_1',
        reference_id: 'checkout_order_1',
        amount: 2999,
        currency: 'USD',
        ...
        sale_transaction_id: 'sale_tx_1',
        webhook_deliveries_queued: 1,
        webhook_deliveries_delivered: 1,
      },
      webhook_delivery_outcomes: [{ id: 'delivery_1', outcome: 'delivered' }],
    })
    const sdk = createClient(completeFn)
    const completed = await sdk.sandbox.completeCheckout({
      checkoutSessionId: 'sess_1',
      idempotencyKey: 'sandbox_complete_order_1',
      paymentId: 'sandbox_pay_1',
    })

    expect(String(completeFn.mock.calls[0][0])).toContain('/sandbox/checkouts/sess_1/complete')
    expect(JSON.parse(completeFn.mock.calls[0][1].body)).toMatchObject({
      idempotency_key: 'sandbox_complete_order_1',
      payment_id: 'sandbox_pay_1',
    })
    expect(completed.checkoutSession?.saleTransactionId).toBe('sale_tx_1')
    expect(completed.checkoutSession?.webhookDeliveriesDelivered).toBe(1)
    expect(completed.webhookDeliveryOutcomes?.[0].outcome).toBe('delivered')

    const failFn = mockFetch({
      success: true,
      checkout_session: {
        id: 'sess_2',
        status: 'failed',
        mode: 'session',
        payment_id: null,
        reference_id: 'checkout_order_2',
        amount: 2999,
        currency: 'USD',
        completed_at: null,
      },
    })
    vi.stubGlobal('fetch', failFn)
    const failed = await sdk.sandbox.failCheckout({
      checkoutSessionId: 'sess_2',
      idempotencyKey: 'sandbox_fail_order_2',
      reason: 'card_declined',
    })
    expect(String(failFn.mock.calls[0][0])).toContain('/sandbox/checkouts/sess_2/fail')
    expect(JSON.parse(failFn.mock.calls[0][1].body)).toMatchObject({
      idempotency_key: 'sandbox_fail_order_2',
      reason: 'card_declined',
    })
    expect(failed.checkoutSession?.status).toBe('failed')

    const webhookFn = mockFetch({
      success: true,
      event_type: 'payout.failed',
      webhook_deliveries_queued: 1,
      webhook_deliveries_delivered: 0,
      webhook_deliveries: [{ id: 'delivery_2', event_type: 'payout.failed', status: 'queued' }],
      webhook_delivery_outcomes: [{ id: 'delivery_2', outcome: 'failed' }],
    })
    vi.stubGlobal('fetch', webhookFn)
    const webhookResult = await sdk.sandbox.sendTestWebhook({
      idempotencyKey: 'sandbox_webhook_payout_failed_1',
      eventType: 'payout.failed',
      payload: { payout_id: 'payout_1' },
    })

    expect(String(webhookFn.mock.calls[0][0])).toContain('/sandbox/webhooks/test')
    expect(JSON.parse(webhookFn.mock.calls[0][1].body)).toMatchObject({
      idempotency_key: 'sandbox_webhook_payout_failed_1',
      event_type: 'payout.failed',
      payload: { payout_id: 'payout_1' },
    })
    expect(webhookResult.webhookDeliveries[0].eventType).toBe('payout.failed')
    expect(webhookResult.webhookDeliveryOutcomes?.[0].outcome).toBe('failed')

    const scenarioFn = mockFetch({
      success: true,
      event_type: 'dispute.created',
      webhook_deliveries_queued: 1,
      webhook_deliveries_delivered: 1,
      webhook_deliveries: [{ id: 'delivery_scenario', event_type: 'dispute.created', status: 'delivered' }],
    })
    vi.stubGlobal('fetch', scenarioFn)
    const scenarioResult = await sdk.sandbox.sendScenario({
      scenario: 'dispute.created',
      idempotencyKey: 'sandbox_dispute_order_1',
      source: 'bridge_smoke',
      runId: 'run_1',
    ...
    const scenarioBody = JSON.parse(scenarioFn.mock.calls[0][1].body)
    expect(String(scenarioFn.mock.calls[0][0])).toContain('/sandbox/webhooks/test')
    expect(scenarioBody).toMatchObject({
      idempotency_key: 'sandbox_dispute_order_1',
      event_type: 'dispute.created',
      payload: {
        scenario: 'dispute.created',
    ...
    expect(scenarioResult.webhookDeliveries[0].eventType).toBe('dispute.created')

    const eventsFn = mockFetch({
      success: true,
      count: 1,
      events: [{ id: 'delivery_3', event_type: 'checkout.completed', status: 'delivered' }],
    })
    vi.stubGlobal('fetch', eventsFn)
    const events = await sdk.sandbox.listEvents({ eventType: 'checkout.completed', limit: 5 })
    expect(String(eventsFn.mock.calls[0][0])).toContain('/sandbox/events')
    expect(String(eventsFn.mock.calls[0][0])).toContain('event_type=checkout.completed')
    expect(String(eventsFn.mock.calls[0][0])).toContain('limit=5')
    expect(events.events[0].eventType).toBe('checkout.completed')

    const filteredEventsFn = mockFetch({ success: true, count: 0, events: [] })
    vi.stubGlobal('fetch', filteredEventsFn)
    await sdk.sandbox.listEvents({
      checkoutId: 'checkout_1',
      orderId: 'order_1',
      paymentId: 'sandbox_pay_1',
    })
    expect(String(filteredEventsFn.mock.calls[0][0])).toContain('checkout_id=checkout_1')
    expect(String(filteredEventsFn.mock.calls[0][0])).toContain('order_id=order_1')
    expect(String(filteredEventsFn.mock.calls[0][0])).toContain('payment_id=sandbox_pay_1')

    const cleanupFn = mockFetch({
      success: true,
      mode: 'sandbox',
      dry_run: true,
      scope: 'run',
      ...
      source: 'bridge_smoke',
      run_id: 'run_1',
      counts: { checkout_sessions: 1, webhook_deliveries: 1 },
      estimated_deleted: 2,
    })
    vi.stubGlobal('fetch', cleanupFn)
    const cleanup = await sdk.sandbox.cleanup({
      scope: 'run',
      source: 'bridge_smoke',
      runId: 'run_1',
    })
    expect(String(cleanupFn.mock.calls[0][0])).toContain('/sandbox/cleanup')
    expect(JSON.parse(cleanupFn.mock.calls[0][1].body)).toMatchObject({
      dry_run: true,
      scope: 'run',
      source: 'bridge_smoke',
      run_id: 'run_1',
    })
    expect(cleanup.estimatedDeleted).toBe(2)

    const runCleanupFn = mockFetch({
      success: true,
      mode: 'sandbox',
      dry_run: false,
      scope: 'run',
    ...
    vi.stubGlobal('fetch', runCleanupFn)
    const runCleanup = await sdk.sandbox.cleanup({
      scope: 'run',
      source: 'bridge_smoke',
      runId: 'run_1',
      confirm: true,
    })
    expect(JSON.parse(runCleanupFn.mock.calls[0][1].body)).toMatchObject({
      dry_run: false,
      scope: 'run',
      source: 'bridge_smoke',
      run_id: 'run_1',
    ...
    expect(runCleanup.deleted?.transactions).toBe(2)
  })
  ...
  it('refundRequests.create maps buyer-facing requests to refund-requests', async () => {
    const fn = mockFetch({
      success: true,
      refund_request: {
        id: 'rr_1',
    ...
    const sdk = createClient(fn)
    const result = await sdk.refundRequests.create({
      saleReference: 'order_1',
      amount: 2999,
      reason: 'Buyer requested refund',
      refundFrom: 'both',
      customerId: 'buyer_1',
      customerEmail: 'buyer@example.com',
      idempotencyKey: 'refund_request_order_1',
      metadata: { orderId: 'order_1' },
    })

    const [url, opts] = fn.mock.calls[0]
    const body = JSON.parse(opts.body)
    expect(String(url)).toContain('/refund-requests')
    expect(body).toMatchObject({
      sale_reference: 'order_1',
      amount: 2999,
      reason: 'Buyer requested refund',
      ...
      customer_id: 'buyer_1',
      customer_email: 'buyer@example.com',
      idempotency_key: 'refund_request_order_1',
    })
    expect(result.refundRequest?.id).toBe('rr_1')
    expect(result.refundRequest?.saleReference).toBe('order_1')
    expect(result.refundRequest?.amountCents).toBe(2999)
    expect(result.refundRequest?.status).toBe('pending')
  })

  it('refundRequests.list uses GET filters and maps request rows', async () => {
    const fn = mockFetch({
      success: true,
      count: 1,
      refund_requests: [
    ...
    const sdk = createClient(fn)
    const result = await sdk.refundRequests.list({ status: 'pending', saleReference: 'order_1', customerId: 'buyer_1', limit: 10 })
    ...
    expect(String(url)).toContain('sale_reference=order_1')
    expect(String(url)).toContain('customer_id=buyer_1')
    expect(String(url)).toContain('limit=10')
    expect(result.count).toBe(1)
    expect(result.refundRequests[0].customerId).toBe('buyer_1')
  })

  it('refundRequests review helpers use the approval workflow routes', async () => {
    const approveFn = mockFetch({
      success: true,
      refund_request: {
        id: 'rr_1',
        sale_reference: 'order_1',
    ...
    expect(approved.refund?.transactionId).toBe('txn_refund_1')
    ...
    await sdk.refundRequests.cancel({ refundRequestId: 'rr_3', reason: 'Buyer withdrew request' })
    expect(String(cancelFn.mock.calls[0][0])).toContain('/refund-requests/rr_3/cancel')
  ...
  it('createParticipant maps nested snake_case correctly', async () => {
    const fn = mockFetch({
      success: true,
      participant: {
        id: 'creator_1',
    ...
    const sdk = createClient(fn)
    const result = await sdk.createParticipant({ participantId: 'creator_1',
      displayName: 'Test',
      email: 'test@example.com',
    ...
    expect(result.participant.accountId).toBe('acc_1')
  ...
  it('webhooks.verifySignature validates timestamped signatures', async () => {
    ...
    const header = await buildWebhookSignature(payload, 'wh_sec_slktest', timestamp)

    const isValid = await sdk.webhooks.verifySignature(payload, header, 'wh_sec_slktest', {
  ...
  it('webhooks.verifySignature rejects expired timestamp', async () => {
    ...
    const header = await buildWebhookSignature(payload, 'wh_sec_slktest', oldTimestamp)
    const isValid = await sdk.webhooks.verifySignature(payload, header, 'wh_sec_slktest', {
      toleranceSeconds: 300,
  ...
  it('webhooks.verifySignature rejects legacy sha256= format', async () => {
    const sdk = new Soledgic({ apiKey: API_KEY, baseUrl: BASE_URL })
    const payload = 'test_payload'
    const hex = await hmacHex('legacy_secret', payload)
  ...
  it('webhooks.verifySignature handles ArrayBuffer payload', async () => {
    ...
    const timestamp = 1_762_000_000
    const header = await buildWebhookSignature(payload, 'wh_sec_slktest', timestamp)
    const isValid = await sdk.webhooks.verifySignature(buffer, header, 'wh_sec_slktest', { now: timestamp })
    expect(isValid).toBe(true)
  })
  ...
  it('testing helpers build signed sandbox webhooks and assert checkout payloads', async () => {
    ...
    const webhook = await buildTestWebhook({
      ...
      data: {
        ...
        amount_cents: 2500,
        currency: 'USD',
        sandbox_idempotency_key: 'complete_run_1',
      },
    })
    ...
    expect(assertSandboxCheckoutCompleted(webhook.rawBody)).toMatchObject({
    ...
    expect(payoutFailure).toMatchObject({
      scenario: 'payout.failed',
      payout_id: 'payout_pay_1',
      status: 'failed',
  ...
  it('getWebhookDeliveries supports bridge trace filters', async () => {
    const fn = mockFetch({ success: true, data: [] })
    const sdk = createClient(fn)
    const result = await sdk.webhooks.listDeliveries({
      endpointId: 'wh_1',
      eventType: 'checkout.completed',
      status: 'delivered',
      checkoutId: 'checkout_1',
  ...
  it('replayWebhookDelivery maps immediate replay outcome', async () => {
    const fn = mockFetch({
      success: true,
      ...
      data: {
        ...
        created_at: '2026-03-13T12:00:00Z',
        delivered_at: '2026-03-13T12:00:01Z',
        next_retry_at: null,
        payload: { event: 'checkout.completed' },
      },
      outcomes: [{ id: 'wd_1', outcome: 'delivered' }],
    })
    const sdk = createClient(fn)
    const result = await sdk.webhooks.replayDelivery('wd_1')

    const body = JSON.parse(fn.mock.calls[0][1].body)
    expect(body).toMatchObject({ action: 'replay', delivery_id: 'wd_1' })
    expect(result.data?.status).toBe('delivered')
    expect(result.outcomes?.[0].outcome).toBe('delivered')
  })

  it('rotateWebhookSecret returns the newly-issued secret', async () => {
    const fn = mockFetch({
      success: true,
      data: { secret: 'wh_sec_slknext' },
    ...
    expect(body.endpoint_id).toBe('wh_1')
    expect(result.data.secret).toBe('wh_sec_slknext')
  ...
  it('createLedger rejects without calling the retired public endpoint', async () => {
    const fn = mockFetch({ success: true })
    const sdk = createClient(fn)
  ...
  it('receipts.generate creates a Soledgic-owned receipt reference', async () => {
    const fn = mockFetch({
      success: true,
      receipt_id: 'rcpt_1',
      hosted_url: 'https://soledgic.com/receipts/rcpt_1',
      receipt: {
        id: 'rcpt_1',
        receipt_id: 'rcpt_1',
        status: 'created',
        hosted_url: 'https://soledgic.com/receipts/rcpt_1',
        file_url: 'https://soledgic.com/receipts/rcpt_1',
        checkout_session_id: 'chk_1',
        sale_transaction_id: 'sale_tx_1',
        order_id: 'order_1',
        external_order_id: 'sole_order_1',
        total_amount: 25,
        currency: 'USD',
        created_at: '2026-05-12T00:00:00Z',
        metadata: { source_app: 'sole_society' },
      },
    })
    const sdk = createClient(fn)
    const result = await sdk.receipts.generate({
      checkoutId: 'chk_1',
      orderId: 'order_1',
      externalOrderId: 'sole_order_1',
      metadata: { source_app: 'sole_society' },
    })

    const [url, opts] = fn.mock.calls[0]
    const body = JSON.parse(opts.body)
    expect(String(url)).toContain('/receipts/generate')
    expect(body).toMatchObject({
      checkout_id: 'chk_1',
      order_id: 'order_1',
      external_order_id: 'sole_order_1',
    ...
    expect(result.hostedUrl).toBe('https://soledgic.com/receipts/rcpt_1')
    expect(result.receipt?.checkoutSessionId).toBe('chk_1')
    expect(result.receipt?.externalOrderId).toBe('sole_order_1')
  })

  it('receipts.get fetches a Soledgic-owned receipt by id', async () => {
    const fn = mockFetch({
      success: true,
      receipt: {
        id: 'rcpt_1',
    ...
    const sdk = createClient(fn)
    const result = await sdk.receipts.get('rcpt_1')

    expect(fn.mock.calls[0][1].method).toBe('GET')
    expect(String(fn.mock.calls[0][0])).toContain('/receipts/rcpt_1')
  ...
  it('createInvoice sends line_items and customer_name', async () => {
    ...
    expect(body.discount_amount).toBe(250)
  ...
  it('kyc.submitBusiness sends bundled KYB profile and document evidence', async () => {
    const fn = mockFetch({ success: true, business_kyb: { organization_id: 'org_1', status: 'under_review' } })
    const sdk = createClient(fn)
    await sdk.kyc.submitBusiness({
      businessType: 'llc',
    ...
    const [url, opts] = fn.mock.calls[0]
    expect(url).toContain('/kyc/business')
    expect(opts.method).toBe('POST')
    const body = JSON.parse(opts.body)
    expect(body.business_type).toBe('llc')
    expect(body.legal_name).toBe('Sole Society LLC')
    expect(body.primary_contact.email).toBe('alex@solesociety.example')
  ...
  it('kyc.submitCreator sends creator KYC to participant-scoped endpoint', async () => {
    const fn = mockFetch({ success: true, creator_kyc: { participant_id: 'creator_1', status: 'under_review' } })
    const sdk = createClient(fn)
    await sdk.kyc.submitCreator({
      participantId: 'creator_1',
    ...
    const [url, opts] = fn.mock.calls[0]
    expect(url).toContain('/kyc/creators/creator_1')
    expect(opts.method).toBe('POST')
    const body = JSON.parse(opts.body)
    expect(body.legal_name).toBe('Creator One')
  ...
  it('kyc status helpers use GET endpoints', async () => {
    const fn = mockFetch({ success: true, creator_kyc: { participant_id: 'creator_1', status: 'pending' } })
    const sdk = createClient(fn)
    await sdk.kyc.getCreator('creator_1')

    const [url, opts] = fn.mock.calls[0]
    expect(opts.method).toBe('GET')
    expect(String(url)).toContain('/kyc/creators/creator_1')
  })
  ...
  it('listComplianceAccessPatterns sends hours param and maps response', async () => {
      ...
      patterns: [
        {
          ...
          request_count: 50,
          unique_actions: 3,
          actions: ['checkout-sessions', 'list-periods', 'get-runway'],
          max_risk_score: 2,
          failed_auths: 0,
        },
      ],
    ...
    expect(result.patterns[0].actions).toEqual(['checkout-sessions', 'list-periods', 'get-runway'])
  ...
  describe('POST method contracts', () => {
    ...
    const postMethods: Array<{
      ...
      {
        name: 'submitBusinessKyb',
        call: (sdk) => sdk.submitBusinessKyb({
          businessType: 'llc',
          legalName: 'Sole Society LLC',
      ...
      },
      {
        ...
        call: (sdk) => sdk.submitCreatorKyc({
          participantId: 'creator_1',
          legalName: 'Creator One',
          email: 'creator@example.com',
          dateOfBirth: '1990-01-01',
          address: { line1: '100 Main', city: 'NYC', state: 'NY', postalCode: '10001', country: 'US' },
          documentEvidence: [{ documentType: 'government_id', providerDocumentId: 'doc_1' }],
        }),
  ...
  describe('response mapping invariants', () => {
    ...
    it('destroy() makes all subsequent requests throw', async () => {
      ...
      await expect(sdk.createPayout({ participantId: 'p', amount: 100, referenceId: 'r' }))
    ...
    it('createLedger rejects as a retired compatibility method', async () => {
      const fn = mockFetch({ success: true })
      const sdk = createClient(fn)
      await expect(sdk.createLedger({ businessName: 'Acme', ownerEmail: 'a@test.com' })).rejects.toMatchObject({
        status: 410,
        code: 'ENDPOINT_RETIRED',
      })
  ...
  describe('deep coverage for 90% mutation score', () => {
    ...
    it('listComplianceSecuritySummary maps every field in summary entries', async () => {
      ...
      expect(s.action).toBe('checkout-sessions')
    ...
    it('createWebhookEndpoint maps secret and message', async () => {
      ...
      expect(result.data.secret).toBe('wh_sec_slknewsecret')
      expect(result.data.isActive).toBe(true)
    ...
    it('parseWebhookSignatureHeader ignores legacy sha256= prefix', () => {
    ...
    it('verifyWebhookSignature validates correct v1 signature', async () => {
      ...
      const secret = 'wh_sec_slktestsecret'
    ...
    it('verifyWebhookSignature rejects legacy sha256= signature', async () => {
    ...
    it('rotateWebhookSecret maps secret and message', async () => {
      ...
      expect(result.success).toBe(true)
      expect(result.data.secret).toBe('wh_sec_slkrotated')
    ...
    it('listComplianceAccessPatterns maps all pattern fields', async () => {
      const fn = mockFetch({
        ...
        patterns: [{
          ...
          request_count: 100,
          unique_actions: 5,
          actions: ['checkout-sessions', 'generate-report'],
          max_risk_score: 30,
          failed_auths: 2,
        }],
      })
      ...
      expect(result.patterns[0].actions).toEqual(['checkout-sessions', 'generate-report'])
    ...
    // --- client.ts: createParticipant maps all fields ---

    it('createParticipant maps all response fields', async () => {
      const fn = mockFetch({
        success: true,
        participant: {
          id: 'c_1',
      ...
      const sdk = createClient(fn)
      const result = await sdk.createParticipant({ participantId: 'c_1',
        displayName: 'Creator One',
        email: 'c1@test.com',
      })

      expect(result.success).toBe(true)
      expect(result.participant.id).toBe('c_1')
      expect(result.participant.accountId).toBe('acct_c1')
      expect(result.participant.displayName).toBe('Creator One')
      expect(result.participant.email).toBe('c1@test.com')
      expect(result.participant.defaultSplitPercent).toBe(80)
      expect(result.participant.payoutPreferences).toEqual({ schedule: 'monthly' })
      expect(result.participant.createdAt).toBe('2026-03-10')
    })

    it('createParticipant defaults payoutPreferences to empty object', async () => {
      const fn = mockFetch({
        success: true,
        participant: {
          id: 'c_2',
      ...
      const sdk = createClient(fn)
      const result = await sdk.createParticipant({ participantId: 'c_2',
        displayName: 'C2',
        email: 'c2@test.com',
      })
    ...
    it('createParticipant handles missing participant key', async () => {
      const fn = mockFetch({ success: true })
      const sdk = createClient(fn)
      const result = await sdk.createParticipant({ participantId: 'c_3',
        displayName: 'C3',
        email: 'c3@test.com',
      })

      // participant is {} from || {}
      expect(result.participant.id).toBeUndefined()
      expect(result.participant.payoutPreferences).toEqual({})
    })
    ...
    // --- client.ts: createLedger stays retired ---

    it('createLedger returns a typed retired-endpoint error', async () => {
      const fn = mockFetch({ success: true })
      const sdk = createClient(fn)
      await expect(sdk.createLedger({
        ...
        status: 410,
        code: 'ENDPOINT_RETIRED',
        details: { endpoint: 'create-ledger' },
      })
    ...
    it('createLedger rejects before sending settings', async () => {
      const fn = mockFetch({ success: true })
      const sdk = createClient(fn)
      await expect(sdk.createLedger({
        businessName: 'B',
    ...
    it('createRefund maps all optional request fields', async () => {
      ...
      expect(body.processor_payment_id).toBeUndefined()
